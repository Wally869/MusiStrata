...
This part is internals, the user's only exposure to Instruments is by specifying an instrument name when creating a Track object.  

## InstrumentsLibrary

The Instruments file contains an extension on the Library base class defined in PrimitiveClassesUtils with the following extensions and fields:  

```python
# A sample record looks like this:
Record({'Signal': 0, 'Family': 'Piano', 'Name': 'Acoustic Grand Piano'})

# The extension on the Library class
