...
class DrumsLibraryClass(Library):
    BaseName = "DrumsLibrary"
    Records = None

    # not sure if use Drum or Drums, so using both
    def GetDrumNameFromSignal(self, signal):
        return self.GetFromValueInField("Signal", signal)[0].Name

    def GetSignalFromDrumName(self, drums):
        return self.GetFromValueInField("Name", drums)[0].Signal

    def GetDrumsNameFromSignal(self, signal):
        return self.GetFromValueInField("Signal", signal)[0].Name

    def GetSignalFromDrumsName(self, drums):
        return self.GetFromValueInField("Name", drums)[0].Signal
...
def GetHeightFromDrumsInstrumentName(drums):
