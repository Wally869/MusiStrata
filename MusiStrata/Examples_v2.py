...
for currID in CHORD_PROGRESSION:
    ...
    barNotes = [
        SoundEvent(0.0, 4.0, n) for n in notes
    ]
    bars.append(
        Bar(barNotes)
    )
...
def GenerateExample1():
    sc = Scale("A", "Minor")
    chords = sc.GetChordsProgression(extensions=[None, CE.M7, CE.M9, None, CE.m9, None, None])
    bars = []
    for ch_id in CHORD_PROGRESSION:
        curr_chord = chords[ch_id]
        notes, _ = curr_chord()
