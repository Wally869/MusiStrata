...
def GenerateExample5():
    sc = Scale("C", "Major")
    progression = [0, 4, 5, 3]
    chords = [Chord("M"), Chord("M"), Chord("m"), Chord("M")]
    bars = []
    for currID in range(len(progression)):
        notes = chords[currID](sc.get_note(progression[currID]))
        barNotes = [SoundEvent(0.0, 4.0, n) for n in notes]
        bars.append(Bar(barNotes))
    bass_track = Track(Bars=bars, Instrument="Acoustic Grand Piano")
    melody_track = Track(Bars=bars, Instrument="Acoustic Grand Piano")
    song = Song(Tempo=120, Tracks=[melody_track, bass_track])
    Render(song, "Examples/example5-v2.mid", RenderFormats.MIDI)
